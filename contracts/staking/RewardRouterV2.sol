...
contract RewardRouterV2 is ReentrancyGuard, Governable {
    ...
    function unstakeGmx(uint256 _amount) external nonReentrant {
        _unstakeGmx(msg.sender, amplify, _amount, true);
    }
    ...
    function handleRewards(
        ...
        if (_shouldStakeGmx && gmxAmount > 0) {
            _stakeGmx(account, account, amplify, gmxAmount);
        }
    ...
    function acceptTransfer(address _sender) external nonReentrant {
        ...
        uint256 stakedGmx = IRewardTracker(stakedGmxTracker).depositBalances(_sender, amplify);
        if (stakedGmx > 0) {
            _unstakeGmx(_sender, amplify, stakedGmx, false);
            _stakeGmx(_sender, receiver, amplify, stakedGmx);
        }
        ...
        uint256 glpAmount = IRewardTracker(feeGlpTracker).depositBalances(_sender, alp);
        if (glpAmount > 0) {
            IRewardTracker(stakedGlpTracker).unstakeForAccount(_sender, feeGlpTracker, glpAmount, _sender);
            IRewardTracker(feeGlpTracker).unstakeForAccount(_sender, alp, glpAmount, _sender);

            IRewardTracker(feeGlpTracker).stakeForAccount(_sender, receiver, alp, glpAmount);
