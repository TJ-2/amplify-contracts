...
contract Vault is ReentrancyGuard, IVault {
    ...
    ITokenManager public tokenManagerContract;  // JJ
    ...
    address public tokenManager;       // JJ
    ...
    function initialize(
        ...
        router = _router;
        usdg = _usdg;
        tokenManager = _tokenManager;   // JJ
        tokenManagerContract = ITokenManager(tokenManager); // JJ
    ...
    // JJ new function
     function setTokenManager(address _tokenManager) external {
        _onlyGov();
        tokenManagerContract = ITokenManager(_tokenManager);
    }
    // end of new function
    ...
    //JJ New functions reads token balances from Token manager and updates in vault
    function updateBalancesFromTokenManager() internal {
        for (uint256 i = 0; i < allWhitelistedTokens.length; i++) {
            address tokenAddress = allWhitelistedTokens[i];
            uint256 balanceFromTokenManager = tokenManagerContract.getTokenBalance(tokenAddress);
            tokenBalances[tokenAddress] = balanceFromTokenManager;
        }
    }

    function _transferIn(address _token) private returns (uint256) {        
        uint256 receivedTokens = IERC20(_token).balanceOf(address(this));   
        IERC20(_token).safeTransfer(tokenManager, receivedTokens);          
        tokenManagerContract.receiveTokens(_token, receivedTokens);          
        updateBalancesFromTokenManager();                                   

        return receivedTokens; 
    }
    // end of new functions

   /*  OLD transferIn function
    function _transferIn(address _token) private returns (uint256) {
    ...
    function _transferOut(address _token, uint256 _amount, address _receiver) private {
        tokenManagerContract.getTokens(_token, _amount);  // JJ
        IERC20(_token).safeTransfer(_receiver, _amount);
        // JJ tokenBalances[_token] = IERC20(_token).balanceOf(address(this));
        updateBalancesFromTokenManager(); 
    }

    function _updateTokenBalance(address _token) private {
        // JJ uint256 nextBalance = IERC20(_token).balanceOf(address(this));
        uint256 nextBalance = tokenManagerContract.getTokenBalance(_token); // JJ
        tokenBalances[_token] = nextBalance;
    }

    function _increasePoolAmount(address _token, uint256 _amount) private {
        poolAmounts[_token] = poolAmounts[_token].add(_amount);
        // JJ uint256 balance = IERC20(_token).balanceOf(address(this));
        uint256 balance = tokenManagerContract.getTokenBalance(_token); // JJ
    ...
    function _increaseReservedAmount(address _token, uint256 _amount) private {
        require(tokenManagerContract.pullTokens(_token, _amount),"Vault: Failed to reserve tokens"); // JJ)
    ...
    function _decreaseReservedAmount(address _token, uint256 _amount) private {
        ...
        require(tokenManagerContract.pushTokens(_token, _amount),"Vault: Failed to send tokens"); // JJ)
